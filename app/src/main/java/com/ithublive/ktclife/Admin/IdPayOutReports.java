...
public class IdPayOutReports extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_pay_out_reports);
        ...
        dialog = new Dialog(IdPayOutReports.this);
        ...
        ib_fromdate.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                DatePickerDialog datePickerDialog = new DatePickerDialog(IdPayOutReports.this, new DatePickerDialog.OnDateSetListener() {
                    @Override
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.POST, BaseUrl.ktc_admin_reportsUrl, new Response.Listener<String>() {
            ...
            public void onResponse(String response) {
                ...
                if (response.contains("conn_error")) {
                    dialog.dismiss();
                    Toast.makeText(IdPayOutReports.this, "Server error. Please  try again later...", Toast.LENGTH_SHORT).show();
                }

                if (response.contains("no_tr_found")) {
                    Toast.makeText(IdPayOutReports.this, "No transaction found.", Toast.LENGTH_SHORT).show();
                    dialog.dismiss();
                }
                ...
                if (response.contains("#streg#") && response.contains("#enreg#")) {
                    ...
                    for (int i = 0; i < res_part.length; i++) {
                        ...
                        PayinPayoutModel payinPayoutModel = new PayinPayoutModel(user_id, pay_date, ins_no, t_amount, pay_mode, withdrawal_date);
                        if (!(withdrawal_date.equals(""))) {
                            list_payout.add(payinPayoutModel);
                            totalAmount += Integer.parseInt(t_amount);
                        }
                    }

                    idPayInListAdaptor = new IdPayInListAdaptor(IdPayOutReports.this, list_payout);
                    ...
                    Toast.makeText(IdPayOutReports.this, "Error.", Toast.LENGTH_SHORT).show();
                    dialog.dismiss();
                }
            }
            ...
            public void onErrorResponse(VolleyError error) {
                Toast.makeText(IdPayOutReports.this, "Server error.", Toast.LENGTH_SHORT).show();
                dialog.dismiss();
            }
        }) {
            @Override
            protected Map<String, String> getParams() throws AuthFailureError {
                ...
                map.put("req_for", "SpecficDt_IdpayOut");
                map.put("fm_dt", etFromDate.getText().toString().trim());
                map.put("to_dt", etToDate.getText().toString().trim());
                return map;
            }
        };
        Volley.newRequestQueue(IdPayOutReports.this).add(stringRequest);
    }
    ...
    class internetChecking extends AsyncTask<String, Void, Integer> {
        ...
        protected void onPostExecute(Integer result) {
            if (isConnected()) {
                ...
                if (result == 0) {
                    dialog.dismiss();
                    Toast.makeText(IdPayOutReports.this, "No internet available.", Toast.LENGTH_SHORT).show();
                }
            } else {
                dialog.dismiss();
                Toast.makeText(IdPayOutReports.this, "No internet connection available.", Toast.LENGTH_SHORT).show();
            }
